...
Player introduction(Player player){
  ...
  printf("Is that a yes or a no?(I'll make it a bit easier than very hard if you say no)\n");
  scanf("%s", difficulty);
  int trueOrFalse = 0;
...
Player gym(Player player){
  ...
  if(response[0] == 'E'){
    ...
    printf("So its $10 for one Strength\n");
    ...
    if(purchase*10 <= player.money){
      player.money = player.money - purchase*10;
...
Player door(Player player){
  ...
  int manHealth = 2000+400*player.day;
  int playerHealth = player.strength*30;
  int defenseEffect = 0;
  while(1==1){
    ...
    if(action[0] == 'U'){
      ...
      if(action[0] == 'T'){
        ...
        if(player.hourglass > 0){
          ...
          player.hourglass = player.hourglass - 1;
          actionTaken++;
        }else{
      ...
      int damageP = player.strength*2+player.strength*0.03*(randomise(201)-1);
      if(player.difficulty == 1){
        if(damageP >  (2000+400*player.day)/4){
          damageP = manHealth/4;
        }
      }else{
        if(damageP > (200+4*player.day)/2){
          damageP = manHealth/2;
        }
      }
      if(flcEffect > 0){
    ...
    if(actionTaken > 0){
      printf("MY TURN NOW\n");
      printf("TAKE THIS\n");
      int damage = 300+player.day*80+playerHealth/2;
      if(player.difficulty == 1){
        if(hourglassEffect == 0){
          damage = damage + playerHealth/3;
        }
      }
      if(hourglassEffect > 0){
