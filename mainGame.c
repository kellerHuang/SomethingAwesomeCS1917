...
Player home(Player player){
  ...
  if(action[0] == 'S' && action[1] == 'L'){
    ...
    player.energy = 100;
    player.reserve = player.reserve*randomise(5);
  ...
  if(response[0] == 'E'){
    ...
    if(response == 'Y' || response == 'y'){
      if(player.energy >= 50){
        ...
        printf("There we go, you're strength went up by %d\n", increase);
...
Player door(Player player){
  ...
  int manHealth = 200+50*player.day;
